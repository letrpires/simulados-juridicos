...
"""
gerar_questoes_pendentes_api_seguro.py
...
Uso em modo teste, sem chamar API:
    python3 gerar_questoes_pendentes_api_seguro.py --dry-run
"""
...
def candidatos_fontes() -> List[Path]:
    ignorar_partes = {
        "html_simulados",
        "auditoria_final",
...
def pontuar_fonte(item: str, categoria: str, caminho: Path) -> int:
    ...
    # Penaliza extração quando o item não pede extração.
    if "extracao" in nome and "extracao" not in alvo:
        pontos -= 50

    return pontos


def localizar_fonte(pendencia: Pendencia, todos: List[Path]) -> Optional[Path]:
    pontuados = [(pontuar_fonte(pendencia.item, pendencia.categoria, p), p) for p in todos]
...
def processar_pendencia(
    ...
    fonte = localizar_fonte(p, fontes)
    if not fonte:
        return ResultadoArquivo(
            categoria=p.categoria,
            item=p.item,
...
def main() -> None:
    ...
    print("Só copie para questoes_validadas_pdf/ os arquivos com status OK_VALIDADO.")
