...
function renderStudy(){const mods=[...new Set(questions.map(q=>q.modulo))].sort(); const cats=[...new Set(questions.map(q=>q.categoria))].sort(); document.getElementById('content').innerHTML=`<div class="card"><h3>Monte sua sessão</h3><div class="filters"><input id="search" placeholder="Buscar enunciado, tema, fonte..."><select id="cat"><option value="">Todas categorias</option>${cats.map(x=>`<option>${x}</option>`).join('')}</select><select id="mod"><option value="">Todos módulos</option>${mods.map(x=>`<option>${x}</option>`).join('')}</select><select id="mode"><option value="all">Todas</option><option value="unseen">Não vistas</option><option value="wrong">Erradas</option><option value="marked">Marcadas</option></select></div><br><button class="primary-btn" onclick="startSessionFromFilters()">Começar</button></div>`}
function filtered(){const term=(document.getElementById('search')?.value||'').toLowerCase();const cat=document.getElementById('cat')?.value||'';const mod=document.getElementById('mod')?.value||'';const mode=document.getElementById('mode')?.value||'all';return questions.filter(q=>{const st=qState(q.id); if(cat&&q.categoria!==cat)return false;if(mod&&q.modulo!==mod)return false;if(mode==='unseen'&&st.seen)return false;if(mode==='wrong'&&st.correct!==false)return false;if(mode==='marked'&&!state.marked[q.id])return false;if(term&&!(`${q.enunciado} ${q.tema} ${q.fonte} ${q.disciplina}`.toLowerCase().includes(term)))return false;return true})}
function startSessionFromFilters(){startSession({list:filtered(),label:document.getElementById('mod').value||document.getElementById('cat').value||'Sessão personalizada'})}
function startSession(opts={}){let list=opts.list;if(!list){if(opts.mode==='continue'&&state.lastSession?.ids?.length)list=state.lastSession.ids.map(id=>questions.find(q=>q.id===id)).filter(Boolean);else list=questions.filter(q=>!qState(q.id).seen).slice(0,30)} session=list.slice(0,50); currentIndex=0; state.lastSession={ids:session.map(q=>q.id)}; save(); renderQuestion()}
